-- Deactivate and soft-delete the current active "Script Generation" v2 prompt
UPDATE Prompts
SET IsActive = 0,
    IsDeleted = 1,
    LastModifiedAt = GETUTCDATE()
WHERE Id = '750F6E26-CFC9-4D18-A213-706B6E78329D';

-- Insert revised "Script Generation" v3 prompt (plain text output, no JSON structure)
INSERT INTO Prompts (Id, Name, Version, SystemPrompt, UserPromptTemplate, Temperature, MaxTokens, TopP, IsActive, IsDeleted, ModelId, CreatedAt, LastModifiedAt)
VALUES (
    NEWID(),
    'Script Generation',
    '3',
    'You are an expert video script writer who writes words meant to come out of someone''s mouth on camera. Not an article. Not a blog post. Actual speech.',
...
After you finish, read the whole thing back to yourself. Every sentence that sounds like writing instead of speech, fix it. Every claim that''s vague where the source material had a specific detail, fix it. Every metaphor that appears once and vanishes, either extend it or cut it.',
    ...
    1,
    0,
    'C9B85660-DAA6-4375-4B72-08DE667D2CED',
    GETUTCDATE(),
    GETUTCDATE()
);
